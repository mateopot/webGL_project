...
const Scene = {
	...
	onMouseDown: (event) => {
		...
		if (Scene.vars.button !== undefined) {
			let intersects = Scene.vars.raycaster.intersectObjects(Scene.vars.button.children, true);
			if (intersects.length > 0) {
				if (!Scene.vars.clicked) {
					Scene.loadMusic();
					Scene.vars.clicked = true;
					Scene.vars.myInterval = setInterval(function () {
						Scene.vars.silverGroup.children[2].traverse(node => {
							if (node.isMesh) {
						...
						Scene.vars.bronzeGroup.children[2].traverse(node => {
							if (node.isMesh) {
								node.material = new THREE.MeshStandardMaterial({
					...
					clearInterval(Scene.vars.myInterval);
					Scene.vars.clicked = false;
				...
				Scene.loadFBX("Socle_Partie1.FBX", 10, [0, 0, 0], [0, 0, 0], 0x1A1A1A, 'socle1', () => {
					Scene.loadFBX("Socle_Partie2.FBX", 10, [0, 0, 0], [0, 0, 0], 0x1A1A1A, 'socle2', () => {
						Scene.loadFBX("Plaquette.FBX", 10, [0, 4, 45], [0, 0, 0], 0xFFFFFF, 'plaquette', () => {
							Scene.loadFBX("button.fbx", 10, [0, 0, 0], [0, 0, 0], 0xFFFFFF, 'boutton', () => {
								Scene.loadText(Scene.vars.text, 10, [0, 23, 52], [0, 0, 0], 0x1A1A1A, "texte", () => {

									let vars = Scene.vars;

									let button = new THREE.Group();
									button.add(vars.boutton);
									button.children[0].traverse(node => {
										if (node.isMesh) {
